...
def image_sketch(decode_array_to_img):
    converted_gray_img=cv2.cvtColor(decode_array_to_img,cv2.COLOR_RGB2GRAY)
    sharpen_gray_img=cv2.bitwise_not(converted_gray_img)
    blur_img=cv2.GaussianBlur(sharpen_gray_img,(111,111),0)
    sharpen_blurimg=cv2.bitwise_not(blur_img)
    sketch_img=cv2.divide(converted_gray_img,sharpen_blurimg,scale=256.0)
